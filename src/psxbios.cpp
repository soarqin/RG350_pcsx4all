...
void psxBios_SetMem() { // 9f
	...
	switch(a0) {
		case 2:
			psxHu32ref(0x1060) = SWAP32(newval);
			psxMu32ref(0x060) = a0;
			SysPrintf("Change effective memory : %d MBytes\n",a0);
			break;

		case 8:
			psxHu32ref(0x1060) = SWAP32(newval | 0x300);
			psxMu32ref(0x060) = a0;
			SysPrintf("Change effective memory : %d MBytes\n",a0);
...
static void buopen(int mcd)
{
	...
	u8 *ptr = (u8*)sioMcdDataPtr(mcd_num);
	u8 *fptr = ptr;
		...
		for (i=1; i<16; i++) {
			...
			for(j=2; j<=nblk; j++) {
				...
				for(i++; i<16; i++) {
					...
					pptr[8] = i - 1;
					pptr[9] = 0;
					for (k=0, xorval=0; k<127; k++) xorval^= pptr[k];
					pptr[127] = xorval;
					pptr = fptr2;
					break;
				}
			...
			pptr[8] = pptr[9] = 0xff;
			for (j=0, xorval=0; j<127; j++) xorval^= pptr[j];
			pptr[127] = xorval;
			SysPrintf("openC %s %d\n", ptr, nblk);
			v0 = 1 + mcd;
...
void psxBios_open() { // 0x32
	...
	if (pa0) {
		if (!strncmp((const char*)pa0, "bu00", 4)) {
			buopen(1);
		}

		if (!strncmp((const char*)pa0, "bu10", 4)) {
			buopen(2);
		}
	}
...
#define bufile(mcd) { \
	char *mcd_data = sioMcdDataPtr((mcd==1) ? MCD1 : MCD2); \
	size_t size_of_name = strlen_internal(dir->name); \
	while (nfile < 16) { \
		int match=1; \
 \
		ptr = mcd_data + 128 * (nfile + 1); \
		nfile++; \
...
void psxBios_firstfile() { // 42
	...
	if (pa0) {
		...
		if (!strncmp((const char*)pa0, "bu00", 4)) {
			...
			bufile(1);
		} else if (!strncmp((const char*)pa0, "bu10", 4)) {
			// firstfile() calls _card_read() internally, so deliver it's event
			DeliverEvent(0x11, 0x2);
			bufile(2);
		}
...
void psxBios_rename() { // 44
	...
	if (pa0 && pa1) {
		if (!strncmp((const char*)pa0, "bu00", 4) && !strncmp((const char*)pa1, "bu00", 4)) {
			burename(1);
		}

		if (!strncmp((const char*)pa0, "bu10", 4) && !strncmp((const char*)pa1, "bu10", 4)) {
			burename(2);
		}
	}
...
void psxBios__card_write() { // 0x4e
	...
	*/
	...
	if (pa2) {
		sioMcdWrite(((port == 0) ? MCD1 : MCD2), (const char*)pa2, a1 * 128, 128);
	}
