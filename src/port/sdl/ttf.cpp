...
void Font::render(SDL_Surface *surface, int x, int y, const char *text) {
	...
	uint16_t *output = static_cast<uint16_t*>(surface->pixels) + stride * y + x;
	...
	while (*text != 0) {
		...
		if (ite == fontCache_.end()) {
			...
			fd = &ite->second;
		uint16_t *outptr = output + fd->ix0 + (fontSize_ + fd->iy0) * stride;
...
Font::FontData *Font::buildCache(uint16_t ch) {
	FontInfo *fi;
#ifdef USE_STB_TRUETYPE
	stbtt_fontinfo *info;
#endif
	...
	for (auto &f: fonts_) {
		...
		if (!FT_Load_Glyph(f.face, index, FT_LOAD_DEFAULT)
			&& !FT_Render_Glyph(f.face->glyph, FT_RENDER_MODE_NORMAL)) break;
#endif
	}
	...
	fd->leftB = static_cast<uint8_t>(fi->fontScale * leftB);
	int ix0, iy0, ix1, iy1;
	...
	fd->iy0 = fontSize_ - slot->bitmap_top;
	...
	auto *src_ptr = slot->bitmap.buffer;
